...
- (void)highlight:(NSRange)range {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (self.lineSpacing) {
                [self.stringStorage addAttribute:NSParagraphStyleAttributeName value:paragraphStyle range:NSMakeRange(0, [self.stringStorage.string length])];
            }
            if (self.font) {
                [self.stringStorage addAttribute:NSFontAttributeName value:self.font range:NSMakeRange(0, [self.stringStorage.string length])];
            }
            [self endEditing];
            [self edited:NSTextStorageEditedAttributes range:range changeInLength:0];
